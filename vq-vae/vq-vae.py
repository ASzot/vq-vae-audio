...
class VQVAE():
    ...
    def __encode_forward_pass(self, x, d):
        k_w = 4
        d_w = 2

        for i in range(1, 7):
            x = Conv1d('conv1d_' + str(i), d, d, k_w, d_w)(x)
            # Add a relu for all but the last layer.
            if i != 6:
                x = tf.nn.relu(x)

        return x

    def __create_embeds(self, K, D):
    ...
    def __discretize(self, x, K, D):
        ...
        _e = tf.reshape(self.embeds, [1, 1, K, D])
        _t = tf.norm(x - _e, axis = -1)
        k = tf.argmin(x, axis = -1) # -> [latent_h,latent_w]
        self.k = k
    ...
    def __decode_backwards_pass(self, x, t, num_layers, num_stages,
            filter_length, width, skip_width):
        ...
        for i in xrange(num_layers):
            ...
            # Condition on z_q
            d = self._condition(d, masked.conv1d(t, num_filters=2*width, filter_length=1, name='cond_map_%d' % (i+1)))
        ...
        s = self._condition(s, masked.conv1d(t, num_filters=skip_width, filter_length=1, name='cond_map_out1'))
        s = tf.nn.relu(s)
    ...
    def __create_loss(self, logits):
        x_indices = tf.cast(tf.reshape(self.x_quantized, [-1]), tf.int32) + 128
        ...
        sg_norm = tf.norm(sg_e - self.z_q, axis=-1) ** 2

        self.vq = tf.reduce_mean(sg_norm, axis=[0,1])
        self.commit = tf.reduce_mean(
            tf.norm(self.z_e - tf.stop_gradient(self.z_q), axis=-1) ** 2,
            axis=[0,1])
        self.loss = self.recon + self.vq + beta * self.commit
        # Not sure why we would need the negative log likelyhood but it was in
        # the original code.
        #self.nll = -1.*(tf.reduce_mean(tf.log(self.p_x_z),axis=[1,2]) + tf.log(1/tf.cast(K,tf.float32)))/tf.log(2.)

    def __create_train_ops(self, enc_param_scope, dec_param_scope, lr):
        # Decoder Grads
        ...
        # Embedding Grads
        embed_grads = list(zip(tf.gradients(self.vq,self.embeds),[self.embeds]))
    ...
    def __init__(self, lr, global_step, beta,
        ...
        with tf.variable_scope('forward') as forward_scope:
            x_scaled = self.__preprocess_forward_pass(x)

            with tf.variable_scope('enc') as enc_param_scope:
                self.__create_embeds(K, D)
                _t = self.__encode_forward_pass(x_scaled, D)

            self.__discretize(_t, K, D)

            # Decoder Pass
            # Copy just to be safe...?
            _t = tf.identity(self.z_q)
            ...
            with tf.variable_scope('dec') as dec_param_scope:
                logits = self.__decode_backwards_pass(x_scaled, _t, num_layers, num_stages,
                        filter_length, width, skip_width)

            # Losses
            # CHECK AXES FOR REDUCE MEAN ON RECON LOSS
            logits = tf.reshape(logits, [-1, 256])
            self.__create_loss(logits)


        # Note:
        # For more concise code I just assume we are always training the
        # network.
        with tf.variable_scope('backward'):
            self.__create_train_ops(enc_param_scope, dec_param_scope, lr)


        all_save_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                enc_param_scope.name) + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, dec_param_scope.name)
...
if __name__ == "__main__":
    ...
    with tf.name_scope('create_inputs'):
        ...
        AUDIO_FILE_PATH = '/all_data/VCTK-Corpus'
    ...
    try:
        ...
        net = VQVAE(learning_rate, global_step, beta, audio_batch, 380, 256,
                sess, True)
